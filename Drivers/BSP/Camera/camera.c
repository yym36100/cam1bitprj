...
void Camera_Init_Device(I2C_HandleTypeDef *hi2c, framesize_t framesize) {
	...
	{
		// ov2640 When using high frame rate register configuration, the XCLK clock using MCO1 output may have abnormalities (distorted screen).
		// You can use TIM1 Channel 1 PWM mode to generate a 12Mhz square wave clock.
		// However, after using TIM1 to output the XCLK clock, it conflicts with the backlight PWM of the LCD,
		// so this function automatically sets the LCD to use software PWM control.
